...
flag → `IDAPT_API_URL` env → `~/.idapt/config.json` apiUrl → compiled-in
default `https://idapt.ai`).
...
# Persisted across shells (writes to ~/.idapt/credentials.json)
idapt config set api-key uk_dev_...
```
...
`microvm-hostd` is developed in `services/microvm-hostd/` with its own Go
module and scripts (`npm run hostd:build`, `npm run hostd:test`). It is not a
CLI subcommand.
