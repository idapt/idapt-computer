...
var shareCmd = &cobra.Command{
	Use:   "share",
	Short: "Manage resource sharing (chats, files, agents, workspaces)",
}
...
var shareListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		return f.ListFormatter("No shared resources found.").WriteList(resp.Data, []output.Column{
			{Header: "ID", Field: "id"},
...
var shareCreateCmd = &cobra.Command{
	...
	Long:  "resource-type: chat | agent | file | workspace. Pass --actor-id (the grantee's profile resourceId) and --permission read|write|admin.",
	Args:  cobra.ExactArgs(2),
	RunE: func(cmd *cobra.Command, args []string) error {
		f := cmdutil.FactoryFromCmd(cmd)
		client, err := f.APIClient()
...
func init() {
	shareListCmd.Flags().String("type", "", "Filter by resource type (chat|agent|file|workspace)")
