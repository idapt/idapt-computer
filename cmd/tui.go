...
Availability: the entire TUI surface is gated on the ` + "`Interactive CLI TUI`" + `
feature flag. When the flag is off the command is absent from the registered
tree (no entry in ` + "`--help`" + `, no shell completion suggestion); when on,
this command launches the TUI immediately.`,
...
When:
  ...
  - You want to use the same models / agents / workspaces you'd use in the web app,
