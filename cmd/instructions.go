...
Examples:
  idapt-computer instructions               — instructions index
  idapt-computer instructions drive         — instructions for the drive resource
  idapt-computer instructions drive delete  — same body as 'instructions drive', plus a footer noting it's resource-scoped

Equivalent flag form on any command:
  idapt-computer drive delete --instructions
...
func printInstructionsIndex(root *cobra.Command) error {
	...
	fmt.Fprintln(out, "# Idapt Computer — instructions index")
	fmt.Fprintln(out)
	fmt.Fprintln(out,
		"Each command has a CONTRACT (use --help / `idapt-computer help <cmd>`) and INSTRUCTIONS (use --instructions / `idapt-computer instructions <cmd>`).")
	fmt.Fprintln(out)
...
func printCommandInstructions(cmd *cobra.Command) {
	...
	if body == "" {
		...
		fmt.Fprintln(out)
		fmt.Fprintf(out, "For args, types, and examples: `idapt-computer help %s`.\n", trimRoot(resource))
		return
	}
	fmt.Fprintln(out, body)
	fmt.Fprintln(out)
	fmt.Fprintln(out, "---")
	fmt.Fprintf(out, "For args / flags: `idapt-computer help %s`.\n", trimRoot(resource))
	if verbScoped {
		fmt.Fprintln(out)
		fmt.Fprintf(out,
			"_Instructions are resource-scoped — same body as `idapt-computer instructions %s`._\n",
			trimRoot(resource))
	}
}
...
func appendInstructionsFooter(cmd *cobra.Command) {
	...
	fmt.Fprintln(out)
	fmt.Fprintf(out,
		"Run `idapt-computer instructions %s` for usage guidance, anti-patterns, and the resource playbook.\n",
		trimRoot(resource),
	)
}
