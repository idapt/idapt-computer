...
func serviceUp(cmd *cobra.Command, reinstall bool) error {
	...
	taskCmd, err := windowsDaemonCommand(exePath, logPath)
	if err != nil {
		return err
	}
...
func installWindowsUpdateTask(exePath string) error {
	...
	taskCmd := fmt.Sprintf(`"%s" update --quiet`, exePath)
	return runSchtasks("/Create", "/SC", "HOURLY", "/MO", "6", "/TN", windowsUpdateTaskName, "/TR", taskCmd, "/F")
}
...
func windowsDaemonCommand(exePath, logPath string) (string, error) {
	...
	return fmt.Sprintf(`cmd /c ""%s" serve >> "%s" 2>&1"`, exePath, logPath), nil
}
