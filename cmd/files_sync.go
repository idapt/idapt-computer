...
import (
	...
	isync "github.com/idapt/idapt-computer/internal/sync"
	"github.com/spf13/cobra"
)

var filesSyncCmd = &cobra.Command{
	Use:   "sync <workspace> <local-path>",
	Short: "Sync Drive (cloud files) with a local directory",
	Long: `Bidirectional sync between your idapt Drive (cloud files) and a local directory.
...
func runFilesSync(cmd *cobra.Command, args []string) error {
	...
	switch direction {
	case "down":
		return syncDown(ctx, fuseClient, workspaceID, localPath, exclusion, limits, cmd.OutOrStdout())
	case "up":
		return syncUp(ctx, fuseClient, workspaceID, localPath, exclusion, cmd.OutOrStdout())
	case "both":
		if err := syncDown(ctx, fuseClient, workspaceID, localPath, exclusion, limits, cmd.OutOrStdout()); err != nil {
			return err
		}
		return syncUp(ctx, fuseClient, workspaceID, localPath, exclusion, cmd.OutOrStdout())
	default:
...
func syncDown(ctx context.Context, client *ifuse.FuseAPIClient, workspaceID, localPath string, exclusion *isync.ExclusionEngine, limits syncLimits, out io.Writer) error {
	fmt.Fprintf(out, "Syncing down from workspace %s to %s...\n", workspaceID, localPath)
	...
	if err := collectServerFiles(ctx, client, workspaceID, "", "", exclusion, &allFiles); err != nil {
		return err
	}

	if limits.maxFiles > 0 && len(allFiles) > limits.maxFiles {
		return fmt.Errorf("workspace has %d files (limit: %d). Use a cloud computer for large workspaces", len(allFiles), limits.maxFiles)
	}
	var totalSize int64
	...
	if limits.maxSize > 0 && totalSize > limits.maxSize {
		return fmt.Errorf("workspace is %dMB (limit: %dMB). Use a cloud computer for large workspaces", totalSize/(1024*1024), limits.maxSize/(1024*1024))
	}
...
func syncUp(ctx context.Context, client *ifuse.FuseAPIClient, workspaceID, localPath string, exclusion *isync.ExclusionEngine, out io.Writer) error {
	fmt.Fprintf(out, "Syncing up from %s to workspace %s...\n", localPath, workspaceID)
	return syncUpDir(ctx, client, workspaceID, "", localPath, "", exclusion, out)
}

func syncUpDir(ctx context.Context, client *ifuse.FuseAPIClient, workspaceID, parentID, localDir, relativePath string, exclusion *isync.ExclusionEngine, out io.Writer) error {
	...
	for _, entry := range entries {
		...
		if entry.IsDir() {
			folder, err := client.CreateFolder(ctx, workspaceID, parentID, entry.Name())
			if err != nil {
				fmt.Fprintf(out, "  SKIP dir %s (error: %v)\n", relPath, err)
				continue
			}
			if err := syncUpDir(ctx, client, workspaceID, folder.ID, localFilePath, relPath, exclusion, out); err != nil {
				return err
			}
			continue
		}
		...
		if _, err := client.CreateFile(ctx, workspaceID, parentID, entry.Name(), content, mimeType); err != nil {
...
func collectServerFiles(ctx context.Context, client *ifuse.FuseAPIClient, workspaceID, folderID, relativePath string, exclusion *isync.ExclusionEngine, out *[]fileWithPath) error {
	files, err := client.ListFiles(ctx, workspaceID, folderID)
	if err != nil {
	...
	for _, f := range files {
		...
		*out = append(*out, fileWithPath{entry: f, relPath: relPath})
		...
		if f.IsFolder {
			if err := collectServerFiles(ctx, client, workspaceID, f.ID, relPath, exclusion, out); err != nil {
				return err
			}
		}
	}
