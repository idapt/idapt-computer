...
Verbs:
  idapt auth login    Sign in in your browser (or paste an API key)
...
func mapOAuthLoginError(err error) error {
	...
	case errors.Is(err, api.ErrOAuthTimedOut):
		return errors.New("timed out waiting for browser sign-in — run `idapt auth login --web` again, or use `idapt auth login` (device code) on headless machines")
...
func mapDeviceLoginError(err error) error {
	...
	case errors.Is(err, api.ErrDeviceCodeExpired):
		return errors.New("the login code expired before approval — run `idapt auth login` again")
