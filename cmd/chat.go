...
plan-mode tool restrictions, project membership, and any other
...
for any account that has the right subscription and project access.
`
...
Flags:
  ...
  --timeout <seconds>            How long to wait for completion (default 120, max 300).`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if streamFlag, _ := cmd.Flags().GetBool("stream"); streamFlag {
...
func init() {
	...
	chatSendCmd.Flags().Bool("stream", false, "Stream the response via SSE (mutually exclusive with --no-wait)")
