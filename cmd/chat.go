...
var chatListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		rows, hasMore, err := f.FetchList(cmd.Context(), cmd, client, "/api/v1/chats", extra)
		if err != nil {
			return err
		}
...
var chatSendCmd = &cobra.Command{
	...
	Example: `  # Send a message and print the reply
  idapt chat send my-chat "Summarize this repo"

  # Pick a model and effort tier for this turn
  idapt chat send my-chat "Tricky question" --model auto --effort-level expert

  # Fire-and-forget (async) — returns a pending token instead of waiting
  idapt chat send my-chat "Long task" --no-wait`,
	ValidArgsFunction: completeChatIDs,
...
var chatMessagesCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		return f.RenderList(cmd, resp.Data, []output.Column{
			{Header: "ID", Field: "id"},
...
var chatRunsCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		rows, hasMore, err := f.FetchList(cmd.Context(), cmd, client, "/api/v1/chats/"+args[0]+"/runs", extra)
		if err != nil {
			return err
		}
...
func init() {
	...
	cmdutil.AddAllFlag(chatListCmd)
	...
	cmdutil.AddAllFlag(chatRunsCmd)
