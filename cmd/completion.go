...
Run ` + "`idapt-computer completion install`" + ` to print a one-liner you can paste
...
Per-shell generators (` + "`idapt-computer completion bash`" + ` etc.) are also
...
func printCompletionInstallHint(cmd *cobra.Command, shell string) error {
	...
	switch shell {
	case "bash":
		fmt.Fprintln(out, "# Enable idapt-computer completion for the current bash session:")
		fmt.Fprintln(out, `source <(idapt-computer completion bash)`)
		fmt.Fprintln(out)
		...
		fmt.Fprintln(out, `sudo sh -c 'idapt-computer completion bash > /etc/bash_completion.d/idapt'`)
		return nil
	case "zsh":
		fmt.Fprintln(out, "# Enable idapt-computer completion for the current zsh session:")
		fmt.Fprintln(out, `source <(idapt-computer completion zsh); compdef _idapt idapt`)
		fmt.Fprintln(out)
		...
		fmt.Fprintln(out, `source <(idapt-computer completion zsh)`)
		fmt.Fprintln(out, `compdef _idapt idapt`)
		fmt.Fprintln(out)
		fmt.Fprintln(out, "# (Optional) System-wide install via the zsh function path:")
		fmt.Fprintln(out, `idapt-computer completion zsh > "${fpath[1]}/_idapt"`)
		return nil
	case "fish":
		fmt.Fprintln(out, "# Enable idapt-computer completion for the current fish session:")
		fmt.Fprintln(out, `idapt-computer completion fish | source`)
		fmt.Fprintln(out)
		fmt.Fprintln(out, "# To enable it for every future session:")
		fmt.Fprintln(out, `idapt-computer completion fish > ~/.config/fish/completions/idapt.fish`)
		return nil
	case "powershell":
		fmt.Fprintln(out, "# Enable idapt-computer completion for the current PowerShell session:")
		fmt.Fprintln(out, `idapt-computer completion powershell | Out-String | Invoke-Expression`)
		fmt.Fprintln(out)
		fmt.Fprintln(out, "# To enable it for every future session, append the same line to your $PROFILE:")
		fmt.Fprintln(out, `idapt-computer completion powershell >> $PROFILE`)
		return nil
	}
	fmt.Fprintln(out, "Could not detect your shell from $SHELL.")
	fmt.Fprintln(out, "Pass an explicit shell name:")
	fmt.Fprintln(out, "  idapt-computer completion install bash")
	fmt.Fprintln(out, "  idapt-computer completion install zsh")
	fmt.Fprintln(out, "  idapt-computer completion install fish")
	fmt.Fprintln(out, "  idapt-computer completion install powershell")
	return nil
}
