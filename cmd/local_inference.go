...
func resolveLocalInferenceComputer(cmd *cobra.Command, f *cmdutil.Factory, positional string) (string, error) {
	...
	if cfg.ComputerID != "" {
		return "", fmt.Errorf("local daemon config at %s is registered but does not contain a public computer id; re-run `idapt-computer up` with a current CLI or pass --computer <computer>", path)
	}
...
func readLocalDaemonConfig(cmd *cobra.Command) (localInferenceDaemonConfig, string, error) {
	...
	if path == "" {
		...
		if err != nil {
			return localInferenceDaemonConfig{}, "", fmt.Errorf("resolve daemon config path: %w", err)
		}
		path = resolved
	}
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return localInferenceDaemonConfig{}, path, fmt.Errorf("no registered Idapt daemon found on this computer; pass --computer <computer> or run `idapt-computer up` first (looked for %s)", path)
		}
...
func parseModelTargetArgs(cmd *cobra.Command, args []string, verb string) (string, string, error) {
	if len(args) < 1 || len(args) > 2 {
		return "", "", fmt.Errorf("usage: idapt-computer local-inference %s <model> [--computer <computer>]", verb)
	}
	if flagged, _ := cmd.Flags().GetString("computer"); flagged != "" {
		if len(args) != 1 {
			return "", "", fmt.Errorf("when --computer is set, use `idapt-computer local-inference %s <model> --computer <computer>`", verb)
		}
		return "", args[0], nil
	}
	if len(args) == 1 {
...
func parseAskTargetArgs(cmd *cobra.Command, args []string) (string, string, string, error) {
	if len(args) < 2 {
		return "", "", "", fmt.Errorf("usage: idapt-computer local-inference ask <model> <prompt> [--computer <computer>]")
	}
	if flagged, _ := cmd.Flags().GetString("computer"); flagged != "" {
...
var localInferenceUpdateCmd = &cobra.Command{
	Use:   "update",
	Short: "Update the managed Ollama engine to the latest version",
	Args:  cobra.MaximumNArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		f := cmdutil.FactoryFromCmd(cmd)
		item, err := localInferenceComputerActionStream(cmd, optionalComputerArg(args), map[string]interface{}{
			"action":  "update",
			"managed": true,
		})
		if err != nil {
...
var localInferenceSetupCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		var resp api.V1ItemResponse
		err = client.Post(cmd.Context(), "/api/v1/ai-gateway/provider-endpoints", map[string]interface{}{
...
func init() {
	...
	localInferenceStartCmd.Flags().Bool("managed", false, "Require the idapt-managed Ollama binary (default: use the managed runtime if installed, else a system ollama on PATH)")
