...
Use this for private self-hosted models attached to an idapt machine daemon.
Runtime traffic stays on that machine: the app publishes daemon commands and
the daemon talks to the local Ollama loopback port.

Recommended setup path:

1. ` + "`local-inference install`" + ` — downloads the managed Ollama bundle into the local daemon user's ` + "`~/.idapt/local-inference`" + ` directory.
...
URL. Use ` + "`--machine <machine>`" + ` to target a different paired daemon.
...
type localInferenceDaemonConfig struct {
	...
	MachineResourceID string `json:"machineResourceId"`
...
func localInferenceMachineAction(cmd *cobra.Command, machineArg string, body map[string]interface{}) (map[string]interface{}, error) {
	f := cmdutil.FactoryFromCmd(cmd)
	client, err := f.APIClient()
	if err != nil {
		return nil, err
	}
	machineID, err := resolveLocalInferenceMachine(cmd, f, machineArg)
	if err != nil {
		return nil, err
	}
	...
	if err := client.Post(cmd.Context(), "/api/v1/machines/"+machineID+"/local-inference", body, &resp); err != nil {
		return nil, err
	}
	return resp.Data, nil
}

func localInferenceMachineActionStream(cmd *cobra.Command, machineArg string, body map[string]interface{}) (map[string]interface{}, error) {
	f := cmdutil.FactoryFromCmd(cmd)
	client, err := f.APIClient()
	if err != nil {
		return nil, err
	...
	resp, err := client.Do(
		cmd.Context(),
		"POST",
		"/api/v1/machines/"+machineID+"/local-inference",
...
func resolveLocalInferenceMachine(cmd *cobra.Command, f *cmdutil.Factory, positional string) (string, error) {
	if flagged, _ := cmd.Flags().GetString("machine"); flagged != "" {
		if positional != "" {
			return "", fmt.Errorf("use either --machine or a positional machine, not both")
		}
		return resolveMachine(cmd, f, flagged)
	}
	if positional != "" {
		return resolveMachine(cmd, f, positional)
	}
	cfg, path, err := readLocalDaemonConfig(cmd)
	if err != nil {
		return "", err
	}
	if cfg.MachineResourceID != "" {
		return cfg.MachineResourceID, nil
	}
	if cfg.Domain != "" {
		if slug := machineSlugFromDomain(cfg.Domain); slug != "" {
			if machineID, err := resolveMachine(cmd, f, slug); err == nil {
				return machineID, nil
			}
		}
	}
	if cfg.MachineID != "" {
		return "", fmt.Errorf("local daemon config at %s is paired but does not contain a public machine id; re-run `idapt pair` with a current CLI or pass --machine <machine>", path)
	}
	return "", fmt.Errorf("local daemon config at %s does not contain a machine id; pass --machine <machine>", path)
}
...
func readLocalDaemonConfig(cmd *cobra.Command) (localInferenceDaemonConfig, string, error) {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return localInferenceDaemonConfig{}, path, fmt.Errorf("no paired Idapt daemon found on this machine; pass --machine <machine> or run `idapt pair --token <token>` first (looked for %s)", path)
		}
...
func machineSlugFromDomain(domain string) string {
...
func optionalMachineArg(args []string) string {
	if len(args) == 0 {
		return ""
	}
	return args[0]
...
func parseModelTargetArgs(cmd *cobra.Command, args []string, verb string) (string, string, error) {
	if len(args) < 1 || len(args) > 2 {
		return "", "", fmt.Errorf("usage: idapt local-inference %s <model> [--machine <machine>]", verb)
	}
	if flagged, _ := cmd.Flags().GetString("machine"); flagged != "" {
		if len(args) != 1 {
			return "", "", fmt.Errorf("when --machine is set, use `idapt local-inference %s <model> --machine <machine>`", verb)
		}
		return "", args[0], nil
	}
	if len(args) == 1 {
...
func parseAskTargetArgs(cmd *cobra.Command, args []string) (string, string, string, error) {
	if len(args) < 2 {
		return "", "", "", fmt.Errorf("usage: idapt local-inference ask <model> <prompt> [--machine <machine>]")
	}
	if flagged, _ := cmd.Flags().GetString("machine"); flagged != "" {
...
func isLikelyLegacyAskTarget(machineArg string, _ string) bool {
	return resolve.IsResourceId(machineArg)
}
...
var localInferenceStatusCmd = &cobra.Command{
	Use:   "status",
	Short: "Show Ollama status through the machine daemon",
	Args:  cobra.MaximumNArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		f := cmdutil.FactoryFromCmd(cmd)
		item, err := localInferenceMachineAction(cmd, optionalMachineArg(args), map[string]interface{}{"action": "status"})
		if err != nil {
			return err
		}
...
var localInferenceInstallCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		item, err := localInferenceMachineActionStream(cmd, optionalMachineArg(args), body)
		if err != nil {
			return err
		}
		return f.Formatter().WriteItem(item, localInferenceColumns())
...
var localInferenceStartCmd = &cobra.Command{
	Use:   "start",
	Short: "Start Ollama through the machine daemon",
	Args:  cobra.MaximumNArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		f := cmdutil.FactoryFromCmd(cmd)
		managed, _ := cmd.Flags().GetBool("managed")
		item, err := localInferenceMachineAction(cmd, optionalMachineArg(args), map[string]interface{}{
			"action":  "start",
			"managed": managed,
		})
		if err != nil {
...
var localInferenceStopCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		f := cmdutil.FactoryFromCmd(cmd)
		item, err := localInferenceMachineAction(cmd, optionalMachineArg(args), map[string]interface{}{"action": "stop"})
		if err != nil {
			return err
		}
		return f.Formatter().WriteItem(item, localInferenceColumns())
...
var localInferenceLogsCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		f := cmdutil.FactoryFromCmd(cmd)
		item, err := localInferenceMachineAction(cmd, optionalMachineArg(args), map[string]interface{}{"action": "logs"})
		if err != nil {
			return err
		}
		if f.Format == output.FormatTable {
...
var localInferenceModelsCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		f := cmdutil.FactoryFromCmd(cmd)
		item, err := localInferenceMachineAction(cmd, optionalMachineArg(args), map[string]interface{}{"action": "list_models"})
		if err != nil {
			return err
		}
...
var localInferencePullCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		f := cmdutil.FactoryFromCmd(cmd)
		machineArg, model, err := parseModelTargetArgs(cmd, args, "pull")
		if err != nil {
			return err
		}
		item, err := localInferenceMachineAction(cmd, machineArg, map[string]interface{}{
			"action": "pull_model",
			"model":  model,
		})
		if err != nil {
...
var localInferenceRemoveCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		f := cmdutil.FactoryFromCmd(cmd)
		machineArg, model, err := parseModelTargetArgs(cmd, args, "remove")
		if err != nil {
			return err
		}
		item, err := localInferenceMachineAction(cmd, machineArg, map[string]interface{}{
...
var localInferenceAskCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		f := cmdutil.FactoryFromCmd(cmd)
		machineArg, model, prompt, err := parseAskTargetArgs(cmd, args)
		if err != nil {
			return err
		}
		item, err := localInferenceMachineAction(cmd, machineArg, map[string]interface{}{
...
var localInferenceSetupCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		machineArg := optionalMachineArg(args)
		machineID, err := resolveLocalInferenceMachine(cmd, f, machineArg)
		if err != nil {
			return err
		}
		...
		if !skipInstall {
			if _, err := localInferenceMachineActionStream(cmd, machineArg, map[string]interface{}{
				"action":   "install",
				"base_url": baseURL,
				"managed":  true,
			}); err != nil {
		...
		if _, err := localInferenceMachineAction(cmd, machineArg, map[string]interface{}{
			"action":   "start",
			"base_url": baseURL,
			"managed":  true,
		}); err != nil {
		...
		return f.Formatter().WriteItem(resp.Data, []output.Column{
			...
			{Header: "MACHINE_ID", Field: "machine_id"},
...
func init() {
	localInferenceCmd.PersistentFlags().String("machine", "", "Target a specific machine instead of the local paired daemon")
