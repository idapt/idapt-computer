...
func resolveLocalInferenceComputer(cmd *cobra.Command, f *cmdutil.Factory, positional string) (string, error) {
	...
	if cfg.ComputerID != "" {
		return "", fmt.Errorf("local daemon config at %s is paired but does not contain a public computer id; re-run `idapt-computer pair` with a current CLI or pass --computer <computer>", path)
	}
...
func readLocalDaemonConfig(cmd *cobra.Command) (localInferenceDaemonConfig, string, error) {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return localInferenceDaemonConfig{}, path, fmt.Errorf("no paired Idapt daemon found on this computer; pass --computer <computer> or run `idapt-computer pair --token <token>` first (looked for %s)", path)
		}
