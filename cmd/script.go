...
var scriptListCmd = &cobra.Command{
	Use:   "list",
	Short: "List scripts in the current workspace",
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		rows, hasMore, err := f.FetchList(cmd.Context(), cmd, client, "/api/v1/drive/files", url.Values{"workspace_id": {workspaceID}})
		if err != nil {
			return err
		}
		...
		err = f.RenderList(cmd, scripts, []output.Column{
			{Header: "ID", Field: "id"},
...
var scriptCreateCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := client.Post(cmd.Context(), "/api/v1/drive/files", body, &resp); err != nil {
			return err
		}
...
var scriptGetCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		id, err := resolveResource(cmd, f, "script", args[0], workspaceID)
		if err != nil {
			return err
		}
		var resp api.V1ItemResponse
		if err := client.Get(cmd.Context(), "/api/v1/drive/files/"+id, nil, &resp); err != nil {
			return err
		}
		return f.Formatter().WriteItem(resp.Data, []output.Column{
			{Header: "ID", Field: "id"},
...
var scriptEditCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := client.Patch(cmd.Context(), "/api/v1/drive/files/"+id, body, nil); err != nil {
			return err
		}
...
var scriptDeleteCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := client.Delete(cmd.Context(), "/api/v1/drive/files/"+id); err != nil {
			return err
		}
...
var scriptRunsCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		rows, hasMore, err := f.FetchList(cmd.Context(), cmd, client, "/api/v1/code-runs", url.Values{"file_id": {id}})
		if err != nil {
			return err
		}
...
func init() {
	...
	cmdutil.AddAllFlag(scriptListCmd)
	...
	cmdutil.AddAllFlag(scriptRunsCmd)
