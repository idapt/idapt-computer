...
var serveCmd = &cobra.Command{
	...
	Long: "Starts the per-machine daemon: heartbeat, the command channel, the " +
		"tunnel client, and FUSE mounts. Public traffic reaches the machine " +
...
func runServe(cmd *cobra.Command, args []string) error {
	...
	log.Printf("idapt %s starting for machine %s", Version, cfg.MachineID)
	...
	if cfg.TunnelProxyURL != "" && cfg.MachineToken != "" {
		tunnelCfg := tunnelclient.NewConfigManager(tunnelclient.DefaultConfigPath)
		tunnelSyncer := tunnelclient.NewSyncer(cfg.AppURL, cfg.MachineID, cfg.MachineToken)
		...
		mux.HandleFunc("/api/tunnels", tunnelclient.NewHandler(tunnelMgr, cfg.MachineToken))
		...
		log.Printf("tunnel: disabled (tunnelProxyUrl or machineToken not set)")
	}
	...
	go func() {
		log.Printf("management API listening on :%s", httpPort)
