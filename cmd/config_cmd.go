...
Valid keys:
  apiUrl              API base URL (default: https://idapt.ai)
  ...
  theme               auto | light | dark — TUI color theme
  lastAgentId         (TUI session memory — usually auto-set)
  lastModelId         (TUI session memory — usually auto-set)
  lastChatId          (TUI session memory — usually auto-set)

API keys are NOT stored here — use ` + "`idapt auth login --api-key <key>`" + `
...
Use this to undo a ` + "`config set`" + ` — e.g. ` + "`idapt config unset defaultWorkspace`" + `
