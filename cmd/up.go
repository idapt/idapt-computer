...
func init() {
	...
	upCmd.Flags().StringVar(&upAutostart, "autostart", "", "Autostart policy: always (boot/login, default), app (desktop-controlled), or manual")
	upCmd.Flags().BoolVar(&upEnableLocalInference, "enable-local-inference", false, "Enable the local-inference (Ollama) command family in the daemon policy at pair time (default-deny otherwise). Interactive runs are prompted.")
...
func runUp(cmd *cobra.Command, args []string) error {
	...
	serviceAutostart = resolveServiceAutostart(upAutostart)
	if installMode == InstallModeSystem {
		serviceAutostart = config.AutostartAlways
	}
	...
	fmt.Fprintln(out, "")
	fmt.Fprintln(out, "Done — paired and daemon autostart installed.")
	fmt.Fprintln(out, "Your computer will show as online in the Computers tab once the")
	fmt.Fprintln(out, "daemon connects (usually a few seconds). If it stays offline, run")
	fmt.Fprintln(out, "`idapt-computer serve` to see why.")
...
func installServiceForMode(cmd *cobra.Command, mode string, reinstall bool) error {
	...
	if serviceAutostart == config.AutostartAlways {
...
func runDeviceFlow(cmd *cobra.Command, appURL, presetCode, configPath, hostname, defaultUser, installMode string) error {
	...
	if err := writeConfigFromCredentials(configPath, appURL, view.ComputerID, view.ComputerToken, view.Domain, view.TunnelProxyURL, defaultUser, resolveEnableLocalInference(cmd)); err != nil {
		return err
	}
	fmt.Fprintln(out, "")
...
func runTokenFlow(cmd *cobra.Command, appURL, token, configPath, hostname, defaultUser, installMode string) error {
	...
	if err := writeConfigFromCredentials(configPath, appURL, result.Data.ComputerID, result.Data.ComputerToken, result.Data.Domain, result.Data.TunnelProxyURL, defaultUser, resolveEnableLocalInference(cmd)); err != nil {
		return err
	}
	fmt.Fprintf(out, "Linked: %s\n", result.Data.Domain)
	return nil
...
func resolveEnableLocalInference(cmd *cobra.Command) bool {
	if upEnableLocalInference {
		return true
	}
	if upYes || upToken != "" || upTokenStdin || upCode != "" || !stdinIsTTY() {
		return false
	}
	answer, err := promptLine(
		cmd.InOrStdin(),
		cmd.ErrOrStderr(),
		"Enable local inference (Ollama) on this computer now? [y/N]: ",
	)
	if err != nil {
		return false
	}
...
func writeConfigFromCredentials(path, appURL, computerID, computerToken, domain, tunnelProxyURL, defaultUser string, enableLocalInference bool) error {
	cfg := map[string]any{}
	if data, err := os.ReadFile(path); err == nil {
		_ = json.Unmarshal(data, &cfg) // best-effort; a corrupt/missing file starts fresh
	}
	cfg["computerId"] = computerID
	cfg["computerResourceId"] = computerID
	cfg["appUrl"] = appURL
	cfg["domain"] = domain
	cfg["jwksUrl"] = appURL + "/api/cloud-computers/jwks"
	cfg["computerToken"] = computerToken
	cfg["tunnelProxyUrl"] = tunnelProxyURL
	cfg["defaultBackendPort"] = 80
	cfg["defaultUser"] = defaultUser
	if enableLocalInference {
		policy, _ := cfg["commandPolicy"].(map[string]any)
		if policy == nil {
			policy = map[string]any{}
		}
		policy["localInference"] = true
		cfg["commandPolicy"] = policy
	}
