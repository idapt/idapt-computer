...
lets you pick a different workspace before approving; a pre-minted pair
token (--token) carries the workspace chosen when it was minted. Share a
...
func init() {
	upCmd.Flags().StringVar(&upToken, "token", "", "Pair token (legacy / CI flow) — INSECURE inline; prefer --token-stdin or IDAPT_TOKEN")
	upCmd.Flags().BoolVar(&upTokenStdin, "token-stdin", false, "Read the pair token from stdin")
...
func runUp(cmd *cobra.Command, args []string) error {
	...
	switch {
	case token != "":
		if err := runTokenFlow(cmd, appURL, token, configPath, hostname, defaultUser); err != nil {
			return err
		}
	default:
	...
	if err := serviceUp(cmd, false); err != nil {
		...
		fmt.Fprintln(out, "Pairing succeeded. Run `idapt-computer service up` manually once the issue is fixed.")
		return nil
	}
	fmt.Fprintln(out, "")
