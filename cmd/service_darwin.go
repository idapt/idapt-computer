...
const macUpdatePlist = `<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
  <key>Label</key>
  <string>ai.idapt.update</string>
  <key>ProgramArguments</key>
  <array>
    <string>%s</string>
  ...
  <key>StandardOutPath</key>
  <string>%s/Library/Logs/Idapt/update.log</string>
  <key>StandardErrorPath</key>
  <string>%s/Library/Logs/Idapt/update.err</string>
</dict>
</plist>
`

const darwinLabel = "ai.idapt.computer"
const darwinUpdateLabel = "ai.idapt.update"
...
func writePlist(runAtLoad bool) (string, error) {
	...
	if err := os.WriteFile(plistPath, []byte(fmt.Sprintf(macPlist, binPath, runAtLoadStr, home, home)), 0o644); err != nil {
		return "", err
	}
...
func serviceUp(cmd *cobra.Command, reinstall bool) error {
	...
	runAtLoad := serviceAutostart == config.AutostartAlways
	if _, statErr := os.Stat(plistPath); reinstall || os.IsNotExist(statErr) {
		if _, err := writePlist(runAtLoad); err != nil {
	...
	if serviceAutostart != config.AutostartManual {
		if err := installDarwinUpdateAgent(cmd, reinstall); err != nil {
			fmt.Fprintf(cmd.ErrOrStderr(), "WARN: could not install auto-update agent: %v\n", err)
		}
	}

	if serviceAutostart == config.AutostartManual {
		fmt.Fprintln(cmd.OutOrStdout(), "Autostart policy: manual (LaunchAgent installed; start with `idapt-computer service restart`).")
		return nil
	}
...
func installDarwinUpdateAgent(cmd *cobra.Command, reinstall bool) error {
	binPath, err := os.Executable()
	if err != nil {
		return err
	}
	...
	plistPath, err := darwinUpdatePlistPath()
	if err != nil {
		return err
	}
	if _, statErr := os.Stat(plistPath); reinstall || os.IsNotExist(statErr) {
		if err := os.WriteFile(plistPath, []byte(fmt.Sprintf(macUpdatePlist, binPath, home, home)), 0o644); err != nil {
			return err
		}
		fmt.Fprintf(cmd.OutOrStdout(), "Auto-update agent installed (%s).\n", plistPath)
	}
	uid := darwinUID()
	_ = exec.Command("launchctl", "bootout", "gui/"+uid, plistPath).Run()
	return exec.Command("launchctl", "bootstrap", "gui/"+uid, plistPath).Run()
}

func darwinUpdatePlistPath() (string, error) {
	home, err := os.UserHomeDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(home, "Library", "LaunchAgents", darwinUpdateLabel+".plist"), nil
}

func removeDarwinUpdateAgent() {
	if plistPath, err := darwinUpdatePlistPath(); err == nil {
		_ = exec.Command("launchctl", "bootout", "gui/"+darwinUID(), plistPath).Run()
...
func collectAutostartFindings() []autostartFinding {
	...
	return darwinAutostartFindings(exists)
}

func repairAutostart(cmd *cobra.Command) error {
	fmt.Fprintln(cmd.OutOrStdout(), "  nothing to repair (macOS uses a single LaunchAgent)")
	return nil
}
