...
var computerListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		extra := url.Values{"workspace_id": {workspaceID}}
		if cmd.Flags().Changed("include-archived") {
			extra.Set("include_archived", "true")
		}
		rows, hasMore, err := f.FetchList(cmd.Context(), cmd, client, "/api/v1/computers", extra)
		if err != nil {
			return err
		}
...
var computerCreateCmd = &cobra.Command{
	...
	Example: `  # Create a pairing token in your personal workspace
  idapt computer create

  # Target a specific workspace
  idapt computer create --workspace my-team

  # List computers and watch one come online
  idapt computer list
  idapt computer start my-box --wait`,
	RunE: func(cmd *cobra.Command, args []string) error {
		f := cmdutil.FactoryFromCmd(cmd)
		client, err := f.APIClient()
		if err != nil {
...
var computerStartCmd = &cobra.Command{
	...
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		return runComputerLifecycle(cmd, args[0], "start", "starting", "running")
	},
}
...
var computerStopCmd = &cobra.Command{
	...
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		return runComputerLifecycle(cmd, args[0], "stop", "stopping", "stopped")
	},
}
...
var computerHibernateCmd = &cobra.Command{
	...
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		return runComputerLifecycle(cmd, args[0], "hibernate", "hibernating", "hibernated")
	},
}
...
func runComputerLifecycle(cmd *cobra.Command, idOrName, path, verbDone, targetState string) error {
	f := cmdutil.FactoryFromCmd(cmd)
	client, err := f.APIClient()
	if err != nil {
		return err
	...
	if wait && targetState != "" {
		final, err := waitForComputerState(cmd, f, client, id, idOrName, targetState)
		if err != nil {
			return err
		}
		return writeComputerItem(f, final)
	}
...
var computerWaitInterval = 3 * time.Second

func waitForComputerState(cmd *cobra.Command, f *cmdutil.Factory, client *api.Client, id, name, targetState string) (map[string]interface{}, error) {
	timeout := 5 * time.Minute
	if cmd.Flags().Lookup("wait-timeout") != nil {
		if d, _ := cmd.Flags().GetDuration("wait-timeout"); d > 0 {
			timeout = d
		}
	}

	sp := f.NewSpinner(fmt.Sprintf("Waiting for %s to be %s", name, targetState))
	sp.Start()
	defer sp.Stop()

	deadline := time.Now().Add(timeout)
	ticker := time.NewTicker(computerWaitInterval)
	defer ticker.Stop()
	...
	for {
		...
		sp.SetLabel(fmt.Sprintf("Waiting for %s to be %s (current: %s)", name, targetState, state))
		if state == targetState {
			return last, nil
		}
		if time.Now().After(deadline) {
			return nil, fmt.Errorf("timed out after %s waiting for %s to reach %q (last state: %q)", timeout, name, targetState, state)
		}
		select {
...
func init() {
	...
	cmdutil.AddAllFlag(computerListCmd)
	...
	for _, c := range []*cobra.Command{computerStartCmd, computerStopCmd, computerHibernateCmd} {
		c.Flags().Bool("wait", false, "Wait until the computer reaches its target state")
		c.Flags().Duration("wait-timeout", 5*time.Minute, "Max time to wait with --wait (e.g. 2m, 30s)")
	}
}
