...
var hubSubmitCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		var resp api.V1ItemResponse
		if err := client.Post(cmd.Context(), "/api/v1/hub/submissions", body, &resp); err != nil {
			return err
		}

		return f.Formatter().WriteItem(resp.Data, []output.Column{
			{Header: "ID", Field: "id"},
			{Header: "STATUS", Field: "submission_status"},
			{Header: "REPO", Field: "repo_url"},
			{Header: "TYPE", Field: "declared_type"},
...
var hubSubmissionsCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := client.Get(cmd.Context(), "/api/v1/hub/submissions/mine", nil, &resp); err != nil {
			return err
		}
