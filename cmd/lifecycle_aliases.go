...
var downCmd = &cobra.Command{
	...
	Long: `Shortcut for ` + "`idapt-computer service down`" + `. Stops the running daemon
without removing the autostart unit, so the next ` + "`idapt-computer up`" + ` is
instant.

To clear the pairing entirely use ` + "`idapt-computer logout`" + `;
to remove the autostart unit too use ` + "`idapt-computer service uninstall`" + `.`,
...
account. The autostart unit stays installed so the next ` + "`idapt-computer up`" + ` is
...
Use ` + "`idapt-computer auth logout`" + ` to clear the CLI's own API key
