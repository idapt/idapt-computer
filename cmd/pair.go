...
var pairCmd = &cobra.Command{
	Use:   "pair",
	Short: "Pair this computer with Idapt (legacy — prefer `idapt-computer up`)",
...
For interactive use, prefer ` + "`idapt-computer up`" + `, which presents a Tailscale-style
device flow (no clipboard-borne secret) and is idempotent across re-runs.
This command is equivalent to ` + "`idapt-computer up --token <token> --config <path>`" + `.
...
The computer lands in the workspace the pair token was minted for — the
web "Add computer" dialog and the desktop auto-pair both default that to
your PERSONAL workspace. Share its local inference into other workspaces
afterwards with ` + "`idapt computer link <computer> --workspace-id <ws>`" + `.`,
	RunE: runPair,
}

func init() {
	pairCmd.Flags().StringVar(&pairToken, "token", "", "Pair token — INSECURE inline; prefer --token-stdin or IDAPT_TOKEN")
	pairCmd.Flags().BoolVar(&pairTokenStdin, "token-stdin", false, "Read the pair token from stdin")
	pairCmd.Flags().StringVar(&pairAppURL, "app-url", "https://idapt.app", "Idapt app URL")
	pairCmd.Flags().StringVar(&pairConfigPath, "config", "/etc/idapt/config.json", "Path to write config")
	rootCmd.AddCommand(pairCmd)
}

func runPair(cmd *cobra.Command, args []string) error {
	token, err := resolvePairToken(cmd, pairToken, pairTokenStdin)
	if err != nil {
		return err
	}
	if token == "" {
		return fmt.Errorf("a pair token is required — pass --token-stdin, --token <token>, or set IDAPT_TOKEN")
	}
	fmt.Fprintln(cmd.ErrOrStderr(), "Note: `idapt-computer pair` is the legacy bootstrap flow. New installs should use `idapt-computer up` (Tailscale-style device flow).")
	...
	if err := os.MkdirAll(strings.TrimSuffix(pairConfigPath, "/config.json"), 0o700); err != nil {
	}
