...
func runChatAsk(cmd *cobra.Command, args []string) error {
	...
	if client == nil {
		return errors.New("no API key configured — run `idapt config set api-key <token>` or set IDAPT_API_KEY")
	}
	...
	if project != "" {
		if resolved, rerr := resolveProjectID(cmd, client, project); rerr == nil {
			project = resolved
		}
	}
	...
	for _, path := range files {
		...
		id, uerr := uploadOne(cmd, client, path, project)
		if uerr != nil {
...
func uploadOne(cmd *cobra.Command, client *api.Client, path, projectID string) (string, error) {
