...
var notificationCmd = &cobra.Command{
	...
	Short:   "Manage notifications (inbox + workspace broadcasts)",
	Long: `Manage notifications via the public v1 API. List your inbox, send
notifications to workspace members, manage channel preferences, and configure
quiet hours.`,
}
var notificationListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := f.RenderList(cmd, resp.Data, []output.Column{
			{Header: "ID", Field: "id"},
		...
		}, "No notifications found."); err != nil {
			return err
		}
...
var notificationGetCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		return formatter.WriteItem(resp.Data, []output.Column{
			...
			{Header: "WORKSPACE", Field: "workspace_id"},
...
var notificationReadCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if len(args) > 0 {
			body := map[string]interface{}{"is_read": true}
...
var notificationSendCmd = &cobra.Command{
	Use:   "send",
	Short: "Send a notification to workspace members",
	Long: `Send a notification to one or more members of a workspace.

Choose your audience with --target (named audience) or --to (explicit
profile ids). Caller must be a member of the workspace. Channel preferences
...
always win.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if cmd.Flags().Changed("deep-link-kind") {
			...
			if v, _ := cmd.Flags().GetString("deep-link-workspace-id"); v != "" {
				dl["workspace_id"] = v
			}
			if v, _ := cmd.Flags().GetString("deep-link-agent-id"); v != "" {
				dl["agent_id"] = v
			}
			if v, _ := cmd.Flags().GetString("deep-link-computer-id"); v != "" {
				dl["computer_id"] = v
			}
...
trio, or pass a JSON body via --json containing {"updates":[...]}.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := client.Patch(cmd.Context(), "/api/v1/notifications/preferences", body, nil); err != nil {
			return err
		}
...
func init() {
	...
	notificationListCmd.Flags().String("workspace-id", "", "Filter by workspace ID")
	...
	notificationSendCmd.Flags().String("deep-link-kind", "", "Deep-link target kind: chat | file | workspace | agent | computer | settings | billing | usage | support-ticket | hub | home")
	...
	notificationSendCmd.Flags().String("deep-link-workspace-id", "", "Required when --deep-link-kind=workspace")
	notificationSendCmd.Flags().String("deep-link-agent-id", "", "Required when --deep-link-kind=agent")
	notificationSendCmd.Flags().String("deep-link-computer-id", "", "Required when --deep-link-kind=computer")
	notificationSendCmd.Flags().String("deep-link-ticket-id", "", "Required when --deep-link-kind=support-ticket")
	notificationSendCmd.Flags().String("deep-link-section", "", "Optional section for settings/workspace/agent/computer/hub kinds")
