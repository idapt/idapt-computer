...
for workspace-wide lookups.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		extra := url.Values{}
		if len(args) > 0 {
			extra.Set("parent_id", args[0])
		}

		rows, hasMore, err := f.FetchList(cmd.Context(), cmd, client, "/api/v1/drive/files", extra)
		if err != nil {
			return err
		}
...
var fileUploadCmd = &cobra.Command{
	...
	Short: "Upload one or more files to your Drive",
	Long: `Upload one or more local files to your Drive.

Accepts multiple paths, so shell globs work directly:

  idapt drive upload *.csv
  idapt drive upload report.pdf notes.md

Directories are skipped with a warning (this command uploads files, not trees).`,
	Example: `  # Upload a single file into the Drive root
  idapt drive upload ./report.pdf

  # Upload several files (shell-expanded) into a folder
  idapt drive upload ./data/*.csv --parent <folder-id>`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, path := range args {
			if err := uploadOneFile(cmd, f, client, path, fields); err != nil {
				return fmt.Errorf("uploading %s: %w", path, err)
			}
			uploaded++
		}
		if uploaded > 1 {
			fmt.Fprintf(cmd.OutOrStdout(), "Uploaded %d files.\n", uploaded)
		}
		return nil
	},
}

func uploadOneFile(cmd *cobra.Command, f *cmdutil.Factory, client *api.Client, path string, fields map[string]string) error {
	fh, err := os.Open(path)
	if err != nil {
	...
	if fi.IsDir() {
		fmt.Fprintf(cmd.ErrOrStderr(), "Skipping directory %s (upload takes files, not directories).\n", path)
		return nil
	}

	bar := f.NewProgressBar("Uploading "+fi.Name(), fi.Size())
	resp, err := client.Upload(cmd.Context(), "/api/v1/drive/files", fi.Name(), bar.ProxyReader(fh), fields)
	if err != nil {
		return err
	}
...
var fileDownloadCmd = &cobra.Command{
	...
	Long: `Download a Drive file to a local path.

Pass "-" as the local path to stream the contents to stdout, so you can pipe
the file onward:

  idapt drive download <file-id> - | grep ERROR
  idapt drive download <file-id> - > local-copy.bin`,
	Example: `  # Download to the current directory (keeps the original name)
  idapt drive download <file-id>

  # Download to a specific path
  idapt drive download <file-id> ./local-copy.pdf
  ...
  idapt drive download <file-id> -`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		defer out.Close()
		bar := f.NewProgressBar("Downloading "+outPath, result.ContentLength)
		n, err := io.Copy(out, bar.ProxyReader(result.Body))
		if err != nil {
			return err
		}
		bar.Done()
...
func init() {
	...
	cmdutil.AddAllFlag(fileListCmd)
