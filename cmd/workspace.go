...
var workspaceListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		return f.ListFormatter("No workspaces found.").WriteList(resp.Data, []output.Column{
			{Header: "ID", Field: "id"},
...
var workspaceUseCmd = &cobra.Command{
	Use:   "use <id-or-slug>",
	Short: "Set the default workspace for subsequent commands",
	Long: `Set the default workspace used when --workspace is omitted.

This is sugar over ` + "`idapt config set defaultWorkspace <slug>`" + `. The value is
validated against your workspaces before saving, so typos fail fast. Clear it
with ` + "`idapt config unset defaultWorkspace`" + ` (or ` + "`idapt workspace use personal`" + `)
to return to your personal workspace.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		fmt.Fprintf(cmd.OutOrStdout(), "Default workspace set to %q (%s).\n", args[0], id)
		return nil
	},
}
...
var workspaceCurrentCmd = &cobra.Command{
	...
	Long: `Show the workspace that scoped commands resolve to right now.

Resolution order: --workspace flag → IDAPT_WORKSPACE env → defaultWorkspace
config → the reserved "personal" workspace.`,
	Args: cobra.NoArgs,
	RunE: func(cmd *cobra.Command, args []string) error {
		f := cmdutil.FactoryFromCmd(cmd)
		client, err := f.APIClient()
		...
		if f.Format == output.FormatTable {
			source := `"personal" (default fallback)`
			switch {
			case globalFlags.Workspace != "":
				source = "--workspace flag"
			case f.Config.DefaultWorkspace != "":
				source = "defaultWorkspace config / IDAPT_WORKSPACE env"
			}
			fmt.Fprintf(cmd.ErrOrStderr(), "Source: %s\n", source)
		}
		id, err := resolveWorkspaceFlag(cmd, f)
		if err != nil {
			return err
		}
		var resp api.V1ItemResponse
