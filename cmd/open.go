...
var openCmd = &cobra.Command{
  ...
  idapt-computer open                        # the app home
  idapt-computer open chat <name-or-id>      # resolve a name first, then open /chats/<id>
  idapt-computer open file <name-or-id>      # resolve a Drive item, then open /drive/<id>
  idapt-computer open --print agent my-agent # print the URL instead of launching
...
instead of trying to launch a browser, so it's pipe-friendly.`,
	Example: `  idapt-computer open
  idapt-computer open chat 3f8hnkm2vp7qsgt0yca4testpr
  idapt-computer open chat my-chat
  idapt-computer open --print agent my-agent`,
...
func runOpen(cmd *cobra.Command, args []string) error {
	...
	switch len(args) {
	case 1:
		return fmt.Errorf("resource URLs are product-owned; use `idapt-computer open <type> <name-or-id>` (for example `idapt-computer open chat %s`)", args[0])
	case 2:
