...
var triggerListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		rows, hasMore, err := f.FetchList(cmd.Context(), cmd, client, "/api/v1/triggers", url.Values{})
		if err != nil {
			return err
		}
...
var triggerRunsCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		rows, hasMore, err := f.FetchList(cmd.Context(), cmd, client, "/api/v1/triggers/"+args[0]+"/runs", url.Values{})
		if err != nil {
			return err
		}
...
func init() {
	...
	cmdutil.AddAllFlag(triggerListCmd)
	cmdutil.AddListFlags(triggerRunsCmd)
	cmdutil.AddAllFlag(triggerRunsCmd)
