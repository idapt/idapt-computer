...
<p align="center">
  ...
  Pair a machine, mount your Drive, expose local ports, and run private
...
Linux/macOS — it uses `~/.local/bin`). For a system-wide install on shared boxes,
Docker images, or CI:
...
idapt ssh my-computer               # SSH into a paired computer through idapt
```

## What it does

- **Per-computer daemon** — pairs this machine to your workspace; heartbeat,
  command channel, firewall.
- **Drive** — `idapt drive mount` for a live FUSE mount of your files + cloud sync.
- **Networking** — `expose` / `unexpose` public tunnels, `ssh`, `tunnel`,
  `firewall`.
...
| `firewall` | Daemon-side firewall rules |
