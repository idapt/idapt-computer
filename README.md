...
Pickers for project / model / agent, slash commands (`/help`, `/new`,
...
user-resource action — chats, agents, projects, files, machines, secrets,
...
| `project` | list, create, get, edit, delete, fork, invitation, member | Project management (incl. fork + slug-based invitations) |
...
| `file` | list, read, write, create, edit, delete, rename, move, mkdir, grep, glob, search, upload, download, mount, unmount | File operations |
| `secret` | list, create, get, edit, delete | Project-scoped credential storage |
...
| `machine` | list, create, get, edit, delete, archive, unarchive, start, stop, test, exec, tmux, file, dir, fw, port, user, env-var | Daemon-backed machine management (commands, files, users, env, firewall) |
| `share` | list, grantees, create, update, delete | Resource sharing (unified `/v1/shares`) |
| `store` | search, install | Hub marketplace (FF42-gated) |
...
| `pair` | | Pair this machine with idapt (daemon bootstrap) |
| `expose` / `unexpose` | | Expose / close a local port as a public tunnel |
| `tunnel` | list, stop | Inspect this machine's tunnels |
| `version` | | Print CLI version |
| `update` | | Self-update binary after SHA256 + Ed25519 signature verification |
| `serve` | | Per-machine daemon (managed-machine systemd service) |
