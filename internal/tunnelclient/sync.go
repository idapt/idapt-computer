...
func NewSyncer(appURL, computerID, computerToken string) *Syncer {
	return &Syncer{
...
func (s *Syncer) Push(ctx context.Context, cfg Config) ([]SyncedTunnel, error) {
	path := fmt.Sprintf("/api/computers/%s/tunnels", s.computerID)
...
func (s *Syncer) MintDaemonToken(ctx context.Context) (string, error) {
	path := fmt.Sprintf("/api/computers/%s/tunnel-token", s.computerID)
...
func (s *Syncer) sign(req *http.Request, method, path string, body []byte) {
	...
	key, err := hex.DecodeString(s.computerToken)
	if err != nil {
		key = []byte(s.computerToken)
	}
	mac := hmac.New(sha256.New, key)
	mac.Write([]byte(msg))
	req.Header.Set("X-Computer-Signature", hex.EncodeToString(mac.Sum(nil)))
