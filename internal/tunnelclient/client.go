...
func NewClient(proxyURL string, tokens *Syncer, ports *ConfigManager, runuserCfg commands.RunuserConfig) *Client {
	return &Client{
		...
		ptySem:     make(chan struct{}, maxPTYSessions()),
	}
}
...
func (c *Client) handleStream(stream *tunnel.Stream, header tunnel.StreamHeader) {
	...
	if header.Kind == tunnel.StreamKindPTY {
		c.handlePTYStream(stream, header)
		return
	}
