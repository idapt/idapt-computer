...
type Client struct {
	...
	httpEnabled bool
	terminal *remoteterminal.Handler
...
func NewClient(proxyURL string, tokens *Syncer, ports *ConfigManager, httpEnabled bool, terminal *remoteterminal.Handler) *Client {
	return &Client{
...
func (c *Client) handleStream(stream *tunnel.Stream, header tunnel.StreamHeader) {
	...
	if header.Kind == tunnel.StreamKindPTY {
		if c.terminal == nil {
			_ = stream.Reject("remote terminal is disabled by local daemon policy")
			return
		}
		c.terminal.HandleStream(stream, header)
		return
	}
	if !c.httpEnabled {
		_ = stream.Reject("tunnels are disabled by local daemon policy")
		return
	}
	if !c.ports.IsExposed(header.Port) {
		_ = stream.Reject(fmt.Sprintf("port %d is not exposed", header.Port))
		return
	}
	local, err := net.DialTimeout("tcp", fmt.Sprintf("127.0.0.1:%d", header.Port), localDialTimeout)
	if err != nil {
		_ = stream.Reject(fmt.Sprintf("dial 127.0.0.1:%d: %v", header.Port, err))
		return
	}
