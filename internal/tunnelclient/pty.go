...
func (c *Client) handlePTYStream(stream *tunnel.Stream, header tunnel.StreamHeader) {
	select {
	case c.ptySem <- struct{}{}:
		defer func() { <-c.ptySem }()
	default:
	...
	sess, err := commands.StartPTYShell(ctx, header.RunAs, header.Mode, header.Cols, header.Rows, c.runuserCfg)
	if err != nil {
