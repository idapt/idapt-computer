...
func (f *Factory) ListFormatter(emptyMessage string) output.Formatter {
	return output.NewList(f.Format, f.Out, f.NoColor, emptyMessage)
}

func (f *Factory) progressEnabled() bool {
	return progress.ShouldRender(f.ErrOut, f.Format == output.FormatTable)
}

func (f *Factory) NewProgressBar(label string, total int64) *progress.Bar {
	return progress.NewBar(f.ErrOut, label, total, f.progressEnabled())
}

func (f *Factory) NewSpinner(label string) *progress.Spinner {
	return progress.NewSpinner(f.ErrOut, label, f.progressEnabled())
}
