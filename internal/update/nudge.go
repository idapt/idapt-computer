...
type CheckState struct {
	...
	LatestVersion string    `json:"latestVersion"` // server form, e.g. "computer-v1.4.2"
	LastNudgedAt  time.Time `json:"lastNudgedAt"`  // when the banner was last shown
	LastManifestCounter int64 `json:"lastManifestCounter"`
}
...
func SaveManifestCounter(counter int64) error {
	st := LoadCheckState()
	if counter <= st.LastManifestCounter {
		return nil
	}
	st.LastManifestCounter = counter
	return writeCheckState(st)
}
...
func Nudge(current, latest string) (string, bool) {
	...
	return fmt.Sprintf("A new idapt-computer version is available: %s → %s. Run `idapt-computer update`.",
