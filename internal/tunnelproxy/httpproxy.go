...
func (s *Server) handlePublic(w http.ResponseWriter, r *http.Request) {
	...
	session, ok := s.hub.Get(entry.MachineID)
	if !ok {
		http.Error(w, "the machine for this tunnel is offline", http.StatusBadGateway)
		return
	}

	if _, ok := s.authenticateVisitor(w, r, host); !ok {
