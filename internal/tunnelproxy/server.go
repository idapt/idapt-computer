...
func (s *Server) handleDaemonConnect(w http.ResponseWriter, r *http.Request) {
	machineID, err := s.authenticateDaemon(r)
	if err != nil {
	...
	if err != nil {
		log.Printf("tunnelproxy: websocket accept (machine %s): %v", machineID, err)
		return
	}
	session, err := tunnel.ServerSession(tunnel.WebSocketNetConn(context.Background(), c))
	if err != nil {
		log.Printf("tunnelproxy: yamux setup (machine %s): %v", machineID, err)
		_ = c.Close(websocket.StatusInternalError, "session setup failed")
		return
	}
	release := s.hub.Register(machineID, session)
	log.Printf("tunnelproxy: daemon connected machine=%s total=%d", machineID, s.hub.Count())

	<-session.CloseChan()
	release()
	_ = session.Close()
	_ = c.Close(websocket.StatusNormalClosure, "session closed")
	log.Printf("tunnelproxy: daemon disconnected machine=%s total=%d", machineID, s.hub.Count())
}

func (s *Server) authenticateDaemon(r *http.Request) (string, error) {
	if s.cfg.DevAuthBypass {
		if mid := r.Header.Get("X-Idapt-Machine-Id"); mid != "" {
			return mid, nil
		}
	}
