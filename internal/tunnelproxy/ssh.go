...
func (s *Server) handleSSHConnect(w http.ResponseWriter, r *http.Request) {
	machineID, err := s.authenticateSSH(r)
	if err != nil {
		log.Printf("tunnelproxy: ssh auth rejected: %v", err)
		http.Error(w, "unauthorized", http.StatusUnauthorized)
		return
	}
	session, ok := s.hub.Get(machineID)
	if !ok {
		http.Error(w, "the machine is offline", http.StatusBadGateway)
		return
	}
	c, err := websocket.Accept(w, r, &websocket.AcceptOptions{InsecureSkipVerify: true})
	if err != nil {
		log.Printf("tunnelproxy: ssh websocket accept (machine %s): %v", machineID, err)
		return
	}
	...
	if err != nil {
		log.Printf("tunnelproxy: ssh stream open (machine %s): %v", machineID, err)
		_ = c.Close(websocket.StatusInternalError, "ssh stream open failed")
		return
	}
	log.Printf("tunnelproxy: ssh session opened machine=%s", machineID)
	...
	log.Printf("tunnelproxy: ssh session closed machine=%s", machineID)
}
...
func (s *Server) authenticateSSH(r *http.Request) (string, error) {
	...
	if claims.Machine == "" {
		return "", errors.New("token has no machine claim")
	}
	return claims.Machine, nil
}
