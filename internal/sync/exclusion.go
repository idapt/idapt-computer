...
func LoadExclusionEngine(workspaceRoot string, extraPatterns []string) *ExclusionEngine {
	idaptsync := readFileContent(filepath.Join(workspaceRoot, ".idaptsync"))
	gitignore := readFileContent(filepath.Join(workspaceRoot, ".gitignore"))
