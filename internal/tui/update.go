...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case pickerSelectMsg:
		...
		case pickerWorkspace:
			m.workspaceID = msg.Item.ID
			m.workspaceName = msg.Item.Label
		}
...
func (m Model) dispatchSlash(p commands.Parsed) (tea.Model, tea.Cmd) {
	...
	case "workspace":
		if len(p.Args) > 0 {
			m.workspaceID = p.Args[0]
			m.workspaceName = p.Args[0]
			m.header.SetState(m.headerState())
			return m, m.persistContext()
		}
		return m.openPicker(pickerWorkspace)
	case "file":
		if len(p.Args) == 0 {
		...
		return m, uploadAttachmentCmd(m.api, m.workspaceID, path)
	case "files":
...
func (m Model) headerState() widgets.HeaderState {
	return widgets.HeaderState{
		Workspace: firstNonEmpty(m.workspaceName, m.workspaceID),
...
func (m Model) persistContext() tea.Cmd {
	...
	if m.workspaceID != "" {
		cfg.DefaultWorkspace = m.workspaceID
	}
	path := m.cfgPath
	return func() tea.Msg {
		if path != "" {
