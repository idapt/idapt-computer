...
func uploadAttachmentCmd(client *api.Client, projectID, path string) tea.Cmd {
	return func() tea.Msg {
		if client == nil {
		...
		resp, err := client.Upload(context.Background(), "/api/v1/files", filepath.Base(path), f, fields)
		if err != nil {
			return fileUploadedMsg{Path: path, Err: err}
		}
