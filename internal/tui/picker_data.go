...
func (m Model) fetchPickerItems(kind pickerKind) tea.Cmd {
	...
	return func() tea.Msg {
		...
		case pickerAgent:
			items, err := loadAgents(ctx, client, workspaceID)
			return pickerLoadedMsg{Kind: kind, Items: items, Err: err}
		case pickerWorkspace:
			items, err := loadWorkspaces(ctx, client)
			return pickerLoadedMsg{Kind: kind, Items: items, Err: err}
		case pickerMenu:
...
func loadAgents(ctx context.Context, client *api.Client, workspaceID string) ([]widgets.PickerItem, error) {
	path := "/api/v1/agents"
	if workspaceID != "" {
...
func loadWorkspaces(ctx context.Context, client *api.Client) ([]widgets.PickerItem, error) {
	var resp api.V1ListResponse
	if err := client.Get(ctx, "/api/v1/workspaces", nil, &resp); err != nil {
		return nil, err
	}
	out := make([]widgets.PickerItem, 0, len(resp.Data))
	for _, row := range resp.Data {
