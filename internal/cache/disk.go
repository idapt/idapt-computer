...
func (dc *DiskCache) WriteLocalConflict(name string, content []byte) error {
	base := filepath.Base(filepath.Clean("/" + name))
	if base == "/" || base == "." || base == ".." || base == "" {
		base = "unnamed"
	}
	localDir := filepath.Join(dc.baseDir, "local")
	if err := os.MkdirAll(localDir, 0o700); err != nil {
		return err
	}
	path := filepath.Join(localDir, base+".conflict")
...
func (dc *DiskCache) Put(fileID string, version int, r io.Reader) (string, error) {
	...
	if err := os.WriteFile(dc.metaPath(fileID), metaData, 0o600); err != nil {
...
func (dc *DiskCache) MarkDirty(fileID string) {
	...
	if elem, ok := dc.items[fileID]; ok {
		...
		if !entry.dirty {
			entry.dirty = true
			dc.dirtyBytes += entry.size
		}
	}
}

func (dc *DiskCache) DirtyBudgetAvailable(additional int64) bool {
	dc.mu.Lock()
	defer dc.mu.Unlock()
	return dc.dirtyBytes+additional <= dc.maxDirtyBytes
}
