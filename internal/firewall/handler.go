...
import (
	...
	"github.com/idapt/idapt-computer/internal/auth"
)

const maxBodySize = 1 << 20 // 1MB

func NewHandler(mgr *Manager, computerToken string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if err := auth.ValidateComputerHMAC(r, computerToken); err != nil {
...
func NewGetHandler(mgr *Manager, computerToken string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if err := auth.ValidateComputerHMAC(r, computerToken); err != nil {
			http.Error(w, `{"error":"forbidden"}`, http.StatusForbidden)
			return
...
func NewIptablesReadHandler(computerToken string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if err := auth.ValidateComputerHMAC(r, computerToken); err != nil {
			http.Error(w, `{"error":"forbidden"}`, http.StatusForbidden)
			return
