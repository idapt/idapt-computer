...
func NewClient(cfg ClientConfig) (*Client, error) {
	if cfg.BaseURL == "" {
		cfg.BaseURL = "https://idapt.app"
	}
	base, err := url.Parse(strings.TrimRight(cfg.BaseURL, "/"))
	if err != nil {
		return nil, fmt.Errorf("invalid base URL: %w", err)
	}
	if err := httpclient.RequireSecureURL(base.String()); err != nil {
		return nil, err
	}
	if cfg.CLIVersion == "" {
		cfg.CLIVersion = "dev"
	...
	return &Client{
		...
		http:       httpclient.New(cfg.CLIVersion, cfg.Timeout),
		errOut:     io.Discard,
	}, nil
}
...
func (c *Client) WithTimeout(timeout time.Duration) *Client {
	...
	return &Client{
		...
		http:       httpclient.New(c.cliVersion, timeout),
		errOut:     c.errOut,
	}
}
