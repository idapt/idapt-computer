...
var (
	...
	ErrOAuthSessionExpired = errors.New("your session has expired — run `idapt auth login` again")
)
...
const callbackHTMLError = `<!doctype html><html><head><meta charset="utf-8"><title>Sign-in failed</title></head>` +
	`<body style="font-family:system-ui;text-align:center;padding-top:4rem">` +
	`<h1>Sign-in failed</h1><p>Return to your terminal and run <code>idapt auth login</code> again.</p></body></html>`
