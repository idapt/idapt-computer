...
func New(format Format, w io.Writer, noColor bool) Formatter {
	...
	default:
		return &TableFormatter{w: w, noColor: noColor, emptyMessage: "No items found."}
	}
}

func NewList(format Format, w io.Writer, noColor bool, emptyMessage string) Formatter {
	fm := New(format, w, noColor)
	if tf, ok := fm.(*TableFormatter); ok && emptyMessage != "" {
		tf.emptyMessage = emptyMessage
	}
	return fm
}
