...
func (c *FuseAPIClient) ListFiles(ctx context.Context, workspaceID, folderID string) ([]FileEntry, error) {
	...
	for {
		query := url.Values{"workspaceId": {workspaceID}, "limit": {pageSize}, "includeHidden": {"true"}}
		if folderID != "" {
...
func (c *FuseAPIClient) CreateFile(ctx context.Context, workspaceID, parentID, name string, content []byte, mimeType string) (*FileEntry, error) {
	ctx, cancel := context.WithTimeout(ctx, 2*time.Minute)
	defer cancel()

	body, contentType, err := buildMultipartForm(workspaceID, parentID, name, content, mimeType)
	if err != nil {
...
func (c *FuseAPIClient) CreateFolder(ctx context.Context, workspaceID, parentID, name string) (*FileEntry, error) {
	ctx, cancel := context.WithTimeout(ctx, c.timeout)
	defer cancel()
