...
func NewSSESubscriber(apiClient *FuseAPIClient, metadataCache *cache.MetadataCache, diskCache *cache.DiskCache, workspaceID string) *SSESubscriber {
...
func (s *SSESubscriber) connect(ctx context.Context) error {
	resp, err := s.apiClient.client.Do(ctx, "GET", "/api/subscriptions/files", nil,
		api.WithQuery(url.Values{"workspaceId": {s.workspaceID}}))
	if err != nil {
		return err
	}
	...
	log.Printf("fuse-sse: connected to workspace %s event stream", s.workspaceID)
