...
func (fh *IdaptFileHandle) Flush(ctx context.Context) syscall.Errno {
	...
	if err := uploadErr; err != nil {
		if err == syscall.ESTALE {
			if writeErr := fh.fuseFS.DiskCache.WriteLocalConflict(fh.entry.Name, content); writeErr != nil {
				log.Printf("fuse-flush: failed to save conflict file: %v", writeErr)
			} else {
				log.Printf("fuse-flush: OCC conflict on %s — saved conflict copy", fh.entry.Name)
			}
			return syscall.ESTALE
		}
		log.Printf("fuse-flush: upload failed for %s: %v", fh.entry.ID, err)
		if !fh.fuseFS.DiskCache.DirtyBudgetAvailable(int64(len(content))) {
			log.Printf("fuse-flush: dirty cache budget exhausted for %s", fh.entry.ID)
...
func (n *IdaptNode) Create(ctx context.Context, name string, flags uint32, mode uint32, out *gofuse.EntryOut) (inode *fs.Inode, fh fs.FileHandle, fuseFlags uint32, errno syscall.Errno) {
	...
	entry, err := n.fuseFS.APIClient.CreateFile(ctx, n.fuseFS.WorkspaceID, n.entry.ID, name, []byte{}, mimeType)
	if err != nil {
