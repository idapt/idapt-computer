...
import (
	...
	"github.com/idapt/idapt-computer/internal/cache"
...
func (mm *MountManager) Mount(ctx context.Context, cfg MountConfig, apiClient *FuseAPIClient) error {
	...
	if cfg.CacheDir == "" {
		...
		if err != nil {
			return fmt.Errorf("default cache dir: %w", err)
		}
		cfg.CacheDir = filepath.Join(root, cfg.WorkspaceID)
	}
...
func DefaultCacheRoot() (string, error) {
	return resolveCacheRoot(os.Geteuid(), os.Getenv("XDG_CACHE_HOME"), userHomeOrEmpty(), os.TempDir())
}

func resolveCacheRoot(euid int, xdgCacheHome, homeDir, tempDir string) (string, error) {
	if euid == 0 {
		return "/var/cache/idapt", nil
	}
	if xdg := strings.TrimSpace(xdgCacheHome); xdg != "" {
		return filepath.Join(xdg, "idapt"), nil
	}
	if home := strings.TrimSpace(homeDir); home != "" {
		return filepath.Join(home, ".cache", "idapt"), nil
	}
	return filepath.Join(tempDir, "idapt-cache"), nil
}
