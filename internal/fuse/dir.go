...
func (n *IdaptNode) Mkdir(ctx context.Context, name string, mode uint32, out *gofuse.EntryOut) (*fs.Inode, syscall.Errno) {
	entry, err := n.fuseFS.APIClient.CreateFolder(ctx, n.fuseFS.WorkspaceID, n.entry.ID, name)
	if err != nil {
...
func (n *IdaptNode) Symlink(ctx context.Context, target, name string, out *gofuse.EntryOut) (*fs.Inode, syscall.Errno) {
	if !isSafeSymlinkTarget(target) {
		log.Printf("fuse-symlink: rejected unsafe target %q for %s", target, name)
		return nil, syscall.EPERM
	}
	entry, err := n.fuseFS.APIClient.CreateFile(ctx, n.fuseFS.WorkspaceID, n.entry.ID, name, []byte(target), "application/x-symlink")
	if err != nil {
