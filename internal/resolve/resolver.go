...
func (r *Resolver) ResolveWorkspace(ctx context.Context, slug string) (string, error) {
	...
	if slug == "" {
		return "", fmt.Errorf("workspace is required; use --workspace or `idapt config set defaultWorkspace <slug>`")
	}
...
func (r *Resolver) Resolve(ctx context.Context, resourceType, name, workspaceID string) (string, error) {
	...
	if len(matches) == 0 {
		return "", fmt.Errorf("%s %q not found in workspace", resourceType, name)
	}
	if len(matches) > 1 {
