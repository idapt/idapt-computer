...
func resolveOllamaDist(version string) (ollamaDist, error) {
	...
	case "linux":
		...
		return ollamaDist{URL: ollamaDownloadBase + "/" + name, ArchiveName: name, Format: archiveTarZst}, nil
	case "darwin":
		const name = "ollama-darwin.tgz"
		return ollamaDist{URL: ollamaDownloadBase + "/" + name, ArchiveName: name, Format: archiveTarGz}, nil
	case "windows":
		if arch != "amd64" && arch != "arm64" {
			return ollamaDist{}, unsupportedOllamaArch(arch)
		}
		name := fmt.Sprintf("ollama-windows-%s.zip", arch)
		return ollamaDist{URL: ollamaDownloadBase + "/" + name, ArchiveName: name, Format: archiveZip}, nil
	default:
		return ollamaDist{}, fmt.Errorf(
