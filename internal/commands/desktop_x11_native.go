...
func (b *x11NativeBackend) Capture(_ context.Context, displayID string) (DesktopCapture, error) {
	conn, err := b.connect()
	if err != nil {
	...
	displays := b.enumerateDisplays(conn)
	var disp DesktopDisplay
	var ok bool
	if displayID == "" {
		disp, ok = findPrimaryDisplay(displays)
	} else {
		disp, ok = findDisplayByID(displays, displayID)
	}
	if !ok {
		return DesktopCapture{}, fmt.Errorf("display %q not found", displayID)
	}

	pngBytes, err := captureRegion(conn, root, disp.OffsetX, disp.OffsetY, disp.Width, disp.Height)
	if err != nil {
		return DesktopCapture{}, err
	}
	out := DesktopCapture{
		...
		ActiveWindowTitle: x11ActiveWindowTitle(conn, root),
	}
	if displayID != "" {
		out.DisplayID = disp.ID
	}
	return out, nil
}

func captureRegion(conn *xgb.Conn, root xproto.Window, x, y, w, h int) ([]byte, error) {
	if w <= 0 || h <= 0 {
		return nil, fmt.Errorf("x11 capture: bad region %dx%d", w, h)
	}
	reply, err := xproto.GetImage(
		conn, xproto.ImageFormatZPixmap, xproto.Drawable(root),
		int16(x), int16(y), uint16(w), uint16(h), 0xffffffff,
	).Reply()
	if err != nil {
