...
func (b *x11NativeBackend) Capture(_ context.Context, displayID string) (DesktopCapture, error) {
	...
	if displayID != "" {
		disp, ok := findDisplayByID(b.enumerateDisplays(conn), displayID)
		if !ok {
			return DesktopCapture{}, fmt.Errorf("display %q not found", displayID)
		}
		x, y, w, h = disp.OffsetX, disp.OffsetY, disp.Width, disp.Height
	}

	pngBytes, err := captureRegion(conn, root, x, y, w, h)
	if err != nil {
		return DesktopCapture{}, err
	}
	out := DesktopCapture{
	...
	if displayID != "" {
		out.OffsetX, out.OffsetY, out.DisplayID = x, y, displayID
	}
	return out, nil
}
