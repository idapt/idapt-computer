...
func runComputerAppComposeUp(ctx context.Context, env *Envelope) Result {
	...
	policyReport, err := validateComposePolicy(composeFile)
	if err != nil {
...
func validateComposePolicy(path string) (composePolicyReport, error) {
	...
	for serviceName, rawService := range services {
		...
		if warnings, err := validateComposeVolumes(serviceName, service["volumes"]); err != nil {
			return report, err
		} else {
...
func validateComposeVolumes(serviceName string, raw any) ([]appPolicyWarning, error) {
	volumes, ok := raw.([]any)
	if !ok {
		return nil, nil
	}
	...
	for _, rawVolume := range volumes {
		...
		case string:
			...
			if isBlockedMountSource(source) {
				return warnings, fmt.Errorf("service %s mounts blocked host path %s", serviceName, source)
			}
		...
		case map[string]any:
			...
			if typ == "bind" && isBlockedMountSource(source) {
				return warnings, fmt.Errorf("service %s mounts blocked host path %s", serviceName, source)
			}
...
func isBlockedMountSource(source string) bool {
	if source == "" {
		return false
	}
	if source == "/var/run/docker.sock" || source == "/" {
		return true
	}
	if strings.HasPrefix(source, "~") || strings.Contains(source, ".ssh") || strings.Contains(source, ".aws") || strings.Contains(source, ".config/gcloud") {
		return true
	}
