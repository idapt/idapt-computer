...
func validateComposePolicy(path string, projectDir string) (composePolicyReport, error) {
	...
	if err := validateComposeTopLevelVolumes(root, projectDir); err != nil {
		return report, err
	}
	if err := validateComposeTopLevelFileMounts(root, projectDir); err != nil {
		return report, err
	}
	return report, nil
}

func validateComposeTopLevelVolumes(root map[string]any, projectDir string) error {
	volumes, ok := root["volumes"].(map[string]any)
	if !ok {
		return nil
	}
	for name, rawVol := range volumes {
		vol, ok := rawVol.(map[string]any)
		if !ok {
			continue
		}
		opts, ok := vol["driver_opts"].(map[string]any)
		if !ok {
			continue
		}
		device := stringValue(opts["device"])
		if device == "" {
			continue
		}
		if err := checkMountSource("volume "+name, device, projectDir); err != nil {
			return fmt.Errorf("top-level volume %q maps a host path into containers: %w", name, err)
		}
	}
	return nil
}

func validateComposeTopLevelFileMounts(root map[string]any, projectDir string) error {
	for _, key := range []string{"secrets", "configs"} {
		section, ok := root[key].(map[string]any)
		if !ok {
			continue
		}
		for name, rawItem := range section {
			...
			if err := checkMountSource(key+" "+name, file, projectDir); err != nil {
				return fmt.Errorf("top-level %s %q reads a host file into containers: %w", key, name, err)
			}
		}
	}
	return nil
