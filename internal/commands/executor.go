...
func NewExecutor(cfg RunuserConfig, deduper *Deduper, capacity int, queue int, health *HealthState, poster ResultPoster) *Executor {
	return NewExecutorWithPolicy(cfg, CommandPolicy{}, deduper, capacity, queue, health, poster)
}

func NewExecutorWithPolicy(cfg RunuserConfig, policy CommandPolicy, deduper *Deduper, capacity int, queue int, health *HealthState, poster ResultPoster) *Executor {
	if capacity <= 0 {
		capacity = 8
	}
	if queue <= 0 {
...
func (e *Executor) Submit(ctx context.Context, env *Envelope) error {
	...
	if !e.policy.Allows(env.Kind) {
		_ = e.resultPoster.Post(context.Background(), Result{
			ID:    env.ID,
			OK:    false,
			Error: &ResultError{Code: ErrPolicyDisabled, Message: fmt.Sprintf("command kind %q is disabled by local daemon policy", env.Kind)},
			Data:  []byte("{}"),
		})
