...
func (b *winNativeBackend) Capture(_ context.Context) ([]byte, int, int, error) {
	vx, vy, vw, vh := b.virtualScreen()
	if vw <= 0 || vh <= 0 {
		return nil, 0, 0, fmt.Errorf("bad virtual screen size %dx%d", vw, vh)
	}
	...
	bmp, _, _ := procCreateCompatBM.Call(screenDC, uintptr(vw), uintptr(vh))
	if bmp == 0 {
		return nil, 0, 0, fmt.Errorf("CreateCompatibleBitmap failed")
	}
	defer procDeleteObject.Call(bmp)
	procSelectObject.Call(memDC, bmp)
	if r, _, _ := procBitBlt.Call(memDC, 0, 0, uintptr(vw), uintptr(vh), screenDC, uintptr(vx), uintptr(vy), srccopy); r == 0 {
	...
	bi.Header.Width = int32(vw)
	bi.Header.Height = -int32(vh) // negative → top-down
	bi.Header.Planes = 1
	bi.Header.BitCount = 32
	bi.Header.Compression = biRGB

	buf := make([]byte, vw*vh*4)
	if r, _, _ := procGetDIBits.Call(memDC, bmp, 0, uintptr(vh),
