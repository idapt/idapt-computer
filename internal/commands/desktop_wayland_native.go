...
func (b *waylandNativeBackend) Capture(_ context.Context) ([]byte, int, int, error) {
	...
	if out, err := cmd.CombinedOutput(); err != nil {
		return nil, 0, 0, fmt.Errorf("grim capture failed: %v: %s", err, strings.TrimSpace(string(out)))
	}
...
func (b *waylandNativeBackend) outputExtent() (int, int) {
	raw, w, h, err := b.Capture(context.Background())
	if err == nil && w > 0 && h > 0 {
		return w, h
	}
	_ = raw
