...
func (c *Client) subscribe(ctx context.Context) error {
	url := fmt.Sprintf("%s/api/computers/%s/stream/commands", c.opts.AppURL, c.opts.ComputerID)
	path := fmt.Sprintf("/api/computers/%s/stream/commands", c.opts.ComputerID)
	...
	keyBytes, err := hex.DecodeString(c.opts.ComputerToken)
	if err != nil {
		keyBytes = []byte(c.opts.ComputerToken)
	}
