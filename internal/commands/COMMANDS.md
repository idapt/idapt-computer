...
shared-kernel hosts (k3d test nodes, busy machines). Runaway process trees are
...
config's public `machineResourceId`; `--machine` targets a different daemon.
...
decoder; host machines do not need a system `zstd` binary.
