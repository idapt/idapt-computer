...
idapt help <resource>                         # Resource's verbs (e.g. `idapt help drive`)
...
In the in-app dispatcher, verbs like `drive delete`, `trigger fire`, `computer manage`, `trigger rotate-secret` refuse the first call with `INSTRUCTIONS_REQUIRED` if the playbook hasn't been fetched this session. The body is returned inline on the failed call — recovery is one extra round-trip, never two. The CLI uses an interactive `--confirm` prompt for the same destructive operations; reading `idapt instructions <res>` first is recommended.

## Resources

`workspace`, `agent`, `file`, `chat`, `task`, `secret`, `notification`, `subagent`, `web`, `media`, `exec`, `model`, `share`, `auth`, `config`, `api-key`. Run `idapt help <resource>` for that resource's verbs.

Some resources require additional features to be enabled on your account (e.g. computers, triggers, desktop). Run `idapt help` to see exactly what's available to you — it filters to your current entitlements.
...
- Workspace scoping via `--workspace <slug>`; defaults to `personal`.
- API key: `--api-key uk_...` per-call, or `idapt auth login --api-key uk_...` to persist it.
- **Resource ids are 26-char Crockford strings** (e.g. `3f8hnkm2vp7qsgt0yca4bz5wrx`). The CLI accepts either a resourceId OR a friendly slug/name everywhere a resource is referenced — names get resolved against the `--workspace`. The response `id` is always the resourceId; internal UUIDs never appear on the wire.
