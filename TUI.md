...
## Availability — feature flag `ff68` "Interactive CLI TUI" (default **off**)
...
feature flag **`ff68`** (display name "Interactive CLI TUI", compiled-in
default: `false`). The flag is opt-in during v1 rollout so support can ramp
users gradually; flipping the server-side flag to `true` for a user enables
the surface immediately on their next CLI invocation.

When the flag is OFF (the default), the gated commands and flags are
**not registered on the Cobra command tree at all**. As a result:

* The bare `idapt` command falls through to printing help (preserving the
  pre-TUI behavior).
* `idapt --help` does not list `tui` and `idapt chat --help` does not list
  `ask` — they are absent from the help table entirely.
* Shell completion (bash / zsh / fish / powershell) does not suggest the
  gated commands or flags.
* Direct invocation returns Cobra's standard `unknown command "tui" for
  "idapt"` or `unknown flag: --stream` message — same UX as typing a
  random typo, no custom "feature is off" prose.
* `idapt chat send` (without `--stream`) continues to work — that path
  predates the TUI work and is never gated.

Flag values are cached at `$XDG_CACHE_HOME/idapt/flags-cache.json` (or the
per-OS equivalent — see `services/idapt/internal/idaptpaths/`) and
refreshed every ~5 min from `GET /api/feature-flags/me`. The CLI consults
the cache once at startup (in `Execute()`, via `registerTUISurface`) to
decide whether to attach the gated subtree — fail-closed in line with
`FlagHub` and `FlagTriggers`, so a pristine install hides the TUI surface
until the flags load completes.

Server-side metadata for the flag lives in `shared/feature-flags/types.ts`
(opaque key `ff68`, displayName "Interactive CLI TUI") and
`backend/lib/feature-flags/server-metadata.ts`. The original opaque key
`ff58` is still owned by **"Cloud Computer Multi-Provider"** (EC2 vs.
Hetzner) — flipping `ff58` does *not* affect the TUI gate.

### History

* Pre-2026-05-27: the CLI gated the TUI surface on `ff58`, which
  collided with the existing "Cloud Computer Multi-Provider" entry in
  `server-metadata.ts`. Symptom: the admin UI showed only one entry
  labelled "Cloud Computer Multi-Provider" (no "Interactive CLI TUI"
  row) and the CLI's "is the TUI flag on?" question was answered by an
  unrelated EC2 toggle. Fix: dedicated `ff68` for the TUI, original
  `ff58` semantics preserved.
...
idapt --output json            # any computer-readable flag also suppresses
```
...
| `/workspace`  | `[slug|id]`         | Switch workspace; no arg opens the picker.          |
...
Model / agent / workspace pickers (which fetch from the API) keep their
existing flow.
...
```
[ Menu ] [ New ] [ Help ] [ Model ] [ Agent ] [ Workspace ] [ Quit ]
```
...
Auth errors render the same multi-line `cmdutil.AuthHint` the CLI prints
elsewhere — TUI, `idapt auth status`, and any one-shot command that hits
a 401 all surface the same recovery instructions.
...
3. `~/.config/idapt/credentials.json` (written by `idapt auth login --api-key …`).

Legacy `mk_*` tokens are ignored (computer-keys; see commit history).
